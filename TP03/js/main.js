...
view.EditableButton.addEventListener("change", function () {
  ...
  localStorage.setItem("editableButtons", JSON.stringify(btnsJson));
});
...
const handleTheme = () => {
  if (calculator.getTheme.value === "os-default") {
    document.body.className = window.matchMedia("(prefers-color-scheme: dark)")
      .matches
      ? "dark"
      : "light";
  } else {
    document.body.className = calculator.getTheme();
  }
  view.theme.value = calculator.getTheme();
};
